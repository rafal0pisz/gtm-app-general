  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error ?? "Failed to disconnect Stape account.");
        return;
      }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16 gap-3" style={{ color: "var(--text-muted)" }}>
        ...
        <span className="text-sm">Checking Stape status...</span>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      {polling && (
        <div className="px-4 py-3 text-sm flex items-center gap-3" style={{ background: "var(--accent-subtle)", border: "1px solid rgba(114,13,214,0.2)", color: "var(--text-secondary)", borderRadius: R }}>
          ...
          Waiting for Stape authorization in the new window...
        </div>
      )}
      ...
      {status?.connected ? (
        <div className="p-5 flex flex-col gap-4" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: R }}>
          ...
          <p className="text-xs" style={{ color: "var(--text-muted)", lineHeight: "1.6" }}>
            Connected to Stape MCP Server. The AI assistant can now manage GTM tags, triggers, and variables through Stape. Authorization is securely stored in Google Secret Manager.
          </p>
        </div>
      ) : (
        <div className="p-10 flex flex-col items-center gap-4 text-center" style={{ background: "var(--surface)", border: "1px dashed var(--border)", borderRadius: R }}>
          ...
          <div>
            <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Not connected to Stape MCP</p>
            <p className="text-xs mt-1 max-w-xs" style={{ color: "var(--text-muted)", lineHeight: "1.6" }}>
              Connect your Stape account to enable the AI assistant to perform GTM operations through the Stape MCP server.
            </p>
          </div>
          <button
          ...
          >
            ...
            {polling ? "Waiting for authorization..." : "Connect to Stape"}
          </button>
        </div>
      )}
    </div>
